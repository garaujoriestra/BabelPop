angular.module("WallapopTFG").controller("adsCtrl", ["$scope", "$state", "$auth", "apiPaths", "APIClient", "userDataStorage", "checkAuth", "states", "errorHandler", function($scope, $state, $auth, apiPaths, APIClient, userDataStorage, checkAuth, states, errorHandler) {
    ...
    APIClient.getAds(apiPaths.getAds).then(
        function(ads) {
            // Actualizamos modelo
            console.log("MODELO ANUNCIOS: ", ads);
            $scope.model = ads;
            // Terminamos carga
