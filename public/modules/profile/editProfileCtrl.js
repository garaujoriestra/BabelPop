angular.module("WallapopTFG").controller("editProfileCtrl", ["$scope", "$rootScope", "$filter", "$timeout", "$state", "states", "apiPaths", "APIClient", "userDataStorage", "checkAuth", "Upload", "edituser", "errorHandler", function($scope, $rootScope, $filter, $timeout, $state, states, apiPaths, APIClient, userDataStorage, checkAuth, Upload, edituser, errorHandler) {
    ...
    $scope.uploadProfilePicture = function(dataUrl, name) {
        ...
        console.log("Ruta donde alojaremos la imagen de perfil", path);
