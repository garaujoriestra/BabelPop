...
router.post("/createAd", upload.single("description"), function(req, res) {
    ...
    if (req.body.title === undefined || req.body.detail === undefined ||
        ...
        if (tipo === "image/png" || tipo === "image/jpeg" || tipo === "image/jpg") {
            ...
            adBody.image = "../images/ads-images/" + req.file.filename + ext;
            adBody.sold = false;
            adBody.reserved = false;
            adBody.visits = 0;
            adBody.likes = 0;
            adBody.purchaser = "";
            ...
            fs.createReadStream("./uploads/" + req.file.filename)
                .pipe(fs.createWriteStream("./public/images/ads-images/" + req.file.filename + ext));
...
router.get("/getAds/:id", function(req, res) {
    ...
    Ad.find({ _id: req.params.id }, function(err, row) {
        ...
        var updateReservoir = checkreservoirList(row[0]);
        updateReservoir = { reservoirList: updateReservoir };

        /*Aprovechamos en este metodo y aumentamos las visitas*/
        console.log('objCount', objCount);
        console.log('updateReservoir', updateReservoir);
        ...
        Ad.update({ _id: row[0]._id }, { $set: updateReservoir }, function(err) {
            if (err) {
                res.status(500).json({
                    result: err.message,
                    status: "Internal server error",
...
function checkreservoirList(list) {
    var reserList = list['reservoirList'] ? list['reservoirList'] : [];
    var actualList = [];
    if (reserList.length > 0) {
        console.log('solo deberia entrar aqui si es mayor que 0', reserList);
        reserList.map(function(c) {
            var now = new Date();
            console.log('antes');
            if (!(now.getTime() > c.userReservation.getTime())) {
                console.log('es menor ahora que la reserva');
                actualList.push(c);
            }
        });
    }
...
router.put("/editAdReserver", function(req, res) {
    var ad = new Ad(req.body);
    Ad.update({ _id: req.body._id }, { $set: ad }, function(err, adUpdated) {
        if (err) {
            return res.status(500).json({
                result: err.message,
                status: "Internal Server Error",
        ...
        res.status(200).json({ ad: adUpdated, status: "OK" });
        return;
    });
});
